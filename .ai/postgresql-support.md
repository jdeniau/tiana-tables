...
| 3   | todo     | `dialect.escapeLiteral` and `booleanLiteral`. `cellValueToSqlLiteral` and `CellContextMenu` stop importing `mysql`                                                                                                                                                                                                                                                                                                                                                                                                          |
| 4   | todo     | **`fix`** `ForeignKeyLink` builds its filter through `buildFilterClause`/`cellValueToSqlLiteral`; the dialect reaches it as a prop from `TableGrid.columnsMeta`, since a hook there would cost two `useContext` per cell. Visible: `?where=col="42"` becomes `col='42'`                                                                                                                                                                                                                                                     |
| 5   | todo     | `src/sql/driver/`: MySQL driver extracted from `#connect`/`#executeQuery`, `loadDriver()` carries the lazy import, `end`/`error` listeners go behind `onClosed`/`onError`, the retry goes through `isConnectionLost`                                                                                                                                                                                                                                                                                                        |
| 6   | todo     | `WriteResult` with its discriminant, `type-guard.ts` deleted. **IPC contract changed — restart the app**                                                                                                                                                                                                                                                                                                                                                                                                                    |
