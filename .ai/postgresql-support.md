...
| 8   | todo     | **`fix`** `Cell.tsx`: the three rendering tiers, `Unknown` → `String(value)`. Visible: a `TIME`/`BIT`/`GEOMETRY`/`BLOB` column shows its value instead of blanking the grid (`YEAR` landed with commit 7). Take `FieldKind.Binary` further while here: measured, `characterSet === 63` on a string or blob type is what separates `BLOB` from `TEXT`, `VARBINARY` from `VARCHAR` and `BINARY` from `CHAR` — the three pairs the wire type confuses — so the kind can say binary before the value is looked at. It stays a derivation inside the driver, never a field on `ResultField`: PostgreSQL answers the same from the `bytea` OID. The value tier is still needed for what no kind covers — MariaDB's `JSON` arriving as a blob, `GEOMETRY` as a plain object                                                                                                              |
