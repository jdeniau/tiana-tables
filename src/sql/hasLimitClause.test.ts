...
describe('hasLimitClause', () => {
  test.each([
    ...
    expect(hasLimitClause(sql, DatabaseEngine.MySQL)).toBe(expected);
  });

  test('a string literal that reads "limit" is not a clause', () => {
    expect(
      hasLimitClause(
        `SELECT * FROM t WHERE name = 'limit'`,
  ...
  test('a column named limit is quoted, so it is not a clause either', () => {
    expect(hasLimitClause('SELECT `limit` FROM t', DatabaseEngine.MySQL)).toBe(
      false
    );
  });
  ...
  test('answers on a query that does not parse', () => {
    expect(
      hasLimitClause('SELECT FROM WHERE LIMIT', DatabaseEngine.MySQL)
    ).toBe(true);
    expect(hasLimitClause('SELECT FROM t1 a', DatabaseEngine.MySQL)).toBe(
      false
    );
  });

  test('answers on an empty query', () => {
    expect(hasLimitClause('', DatabaseEngine.MySQL)).toBe(false);
  });
});
