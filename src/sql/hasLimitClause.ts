 ...
 * (see `parser/index.ts`). A query the user is still writing must still get an
 ...
 */
export function hasLimitClause(sql: string, engine: DatabaseEngine): boolean {
  return getParser(engine)
    .getAllTokens(sql)
